...
import org.kiama.example.iswim.compiler.IswimTree.IswimProg
import org.kiama.example.iswim.compiler.{CodeGenerator, SyntaxAnalyser, SemanticAnalyser}
 ...
 */
object Main extends SyntaxAnalyser with CodeGenerator with CompilerWithConfig[IswimProg,ISWIMConfig] {
    ...
    override def process (filename : String, iswimcode : IswimProg, config : ISWIMConfig) {
        ...
        val messages = analyser.errors (iswimcode)

        if (analyser.isSemanticallyCorrect (iswimcode)) {
